...
EMAIL_PASSWORD = "mgcujjkwfwadpqqk"  # يجب استبدالها بكلمة مرور تطبيق جديدة من جوجل
...
# ------------------------------- دوال البريد الإلكتروني المحسّنة -------------------------------
def send_email_notification(recipient_email, subject, body):
    """إرسال بريد إلكتروني باستخدام SMTP (Gmail) مع معالجة أخطاء دقيقة"""
    try:
        ...
        if not sender_email or not sender_password:
            return False, "⚠️ بيانات البريد الإلكتروني غير موجودة. يرجى إدخالها في الإعدادات."
        ...
        return False, f"❌ فشل المصادقة: تأكد من استخدام كلمة مرور التطبيق (App Password) وليس كلمة المرور العادية. (تفاصيل: {e})"
...
def send_email_to_all(subject, body):
    ...
    if not recipients:
        return False, "❌ لا يوجد مستلمين صالحين."
    
    success_count = 0
    errors = []
    for recipient in recipients:
        success, msg = send_email_notification(recipient, subject, body)
        if success:
            success_count += 1
        else:
    ...
    if success_count == len(recipients):
        return True, f"✅ تم إرسال البريد لجميع المستلمين ({success_count})"
    elif success_count > 0:
        return True, f"⚠️ تم إرسال البريد لـ {success_count} من {len(recipients)} مستلم. الأخطاء: {'; '.join(errors)}"
    else:
        return False, f"❌ فشل إرسال البريد لجميع المستلمين. الأخطاء: {'; '.join(errors)}"

# ------------------------------- دوال بناء محتوى البريد -------------------------------
...
# ------------------------------- دوال إدارة المستخدمين والصلاحيات -------------------------------
...
def save_users_to_github(users_data):
    try:
        ...
        st.error(f"❌ فشل حفظ المستخدمين: {e}")
        return False
...
def login_ui():
    ...
    if not st.session_state.logged_in:
        if st.button("تسجيل الدخول"):
            current_users = load_users_from_github()
    ...
    with st.form(key="add_event_form"):
        ...
        if submitted:
            ...
            if save_and_push_to_github(sheets_edit, commit_message):
                ...
                try:
                    subject = f"عطل جديد: {selected_equipment}"
                    body = build_email_event_message(
                        selected_equipment,
                        event_desc,
                        correction_desc,
                        servised_by,
...
def execute_maintenance_with_date(sheets_edit, equipment_name, task_name, execution_date, performed_by, used_spare_part="", used_quantity=1, image_url=None, section=None):
    ...
    # ==================== إرسال إشعار بريد إلكتروني ====================
    try:
        subject = f"تم تنفيذ صيانة: {equipment_name}"
        body = build_email_maintenance_execution_message(
            equipment_name,
            task_name,
            performed_by,
            ...
            if selected_task:
                ...
                if st.button("✅ تم تنفيذ الصيانة", type="primary"):
                    if not performed_by:
                        ...
                        if success:
                            if link_to_event:
                                # تسجيل الصيانة كحدث عطل
                                target_sheet = None
                                target_df = None
                                for sheet_name, df in sheets_edit.items():
                                    if sheet_name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"]]:
                                        if selected_equipment in get_equipment_list_from_sheet(df):
                                            target_sheet = sheet_name
                                            target_df = df
                                            break
                                if target_sheet is not None:
                                    spare_part_used = f"{part_name} (كمية {consume_qty})" if part_name else ""
                                    new_row = {
                                        "مده الاصلاح": 0,
                                        "التاريخ": execution_date.strftime("%Y-%m-%d"),
                                        "المعدة": selected_equipment,
                                        "الحدث/العطل": f"صيانة وقائية: {selected_task}",
                                        "الإجراء التصحيحي": f"تم تنفيذ الصيانة الدورية '{selected_task}' بواسطة {performed_by}",
...
with st.sidebar:
    ...
    if not st.session_state.get("logged_in"):
        ...
        st.markdown("---")
        with st.expander("📧 إعدادات البريد الإلكتروني", expanded=False):
            temp_sender = st.text_input("📧 البريد المرسل (Gmail)", value=st.session_state["temp_sender_email"])
            temp_pass = st.text_input("🔑 كلمة مرور التطبيق", type="password", value=st.session_state["temp_sender_password"])
            temp_recipients = st.text_area("📌 المستلمين (افصل بفواصل)", value=st.session_state["temp_recipients"])
            if st.button("💾 حفظ الإعدادات", key="save_email_settings"):
            ...
            if st.button("📧 إرسال بريد تجريبي", key="test_email"):
                if temp_sender and temp_pass and temp_recipients:
                    ...
                    with st.spinner("جاري الإرسال..."):
                        success, msg = send_email_to_all(
                            "📧 بريد تجريبي من CMMS",
                            "هذه رسالة تجريبية. إذا وصلتك فهذا يعني أن الإعدادات صحيحة."
                        )
                        if success:
                            st.success(msg)
                            st.balloons()
                    ...
                    st.warning("⚠️ الرجاء إدخال جميع الحقول أولاً.")
...
with tabs[idx]:
    ...
    st.info("🔍 يمكنك البحث في جميع الأقسام باستخدام النصوص والتواريخ.")
    # تبسيط: عرض رسالة فقط
    st.success("✅ تم تحميل البحث المتقدم.")
idx += 1

with tabs[idx]:
    st.subheader("📊 تحليل الأعطال")
    st.info("تحليل الإجراءات التصحيحية المتكررة والفجوات الزمنية.")
    st.success("✅ تم تحميل تحليل الأعطال.")
idx += 1
...
with tabs[idx]:
    ...
    try:
        ...
        st.warning(f"خطأ في فحص الصيانة: {e}")
...
if username == "admin":
    with tabs[idx]:
        ...
        st.info("هنا يمكنك إدارة المستخدمين. (سيتم إضافتها لاحقاً)")
        st.success("✅ تم تحميل إدارة المستخدمين.")
    idx += 1
